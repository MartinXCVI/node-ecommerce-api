...
// method: GET
export const getTotalSalesSum = async (req, res)=> {
...
export const createOrder = async (req, res)=> {
  ...
  const totalPrices = await Promise.all(orderItemsIdsResolved.map(async orderItemId => {
    ...
    const orderItem = await OrderItem.findById(orderItemId).populate('product', 'price') // The populate() methods let's you reference documents in other collections
...
export const updateOrder = async (req, res)=> {
  ...
  try {
    ...
    return res.status(201).json({
      success: true,
      message: `Order ${updatedOrder.name || updatedOrder._id} successfully updated`,
      order: updatedOrder
    })
  } catch(error) {
...
export const deleteOrder = async (req, res)=> {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: `Order '${deletedOrder.name || deletedOrder._id}' successfully deleted`,
    })
  } catch(error) {
