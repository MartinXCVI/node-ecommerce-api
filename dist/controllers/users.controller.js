...
export const userLogin = async (req, res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id, isAdmin: user.isAdmin }, secret, { expiresIn: '15m' } // 15 minutes lifespan for access token
        );
        // Generating refresh token: data, refresh secret, expiration
        const refreshToken = jwt.sign({ userId: user.id }, refreshSecret, { expiresIn: '7d' } // 7 days lifespan for refresh token
        );
...
export const refreshUserToken = async (req, res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id, isAdmin: user.isAdmin }, secret, { expiresIn: "15m" });
