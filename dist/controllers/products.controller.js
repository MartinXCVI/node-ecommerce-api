...
export const updateProductGallery = async (req, res) => {
    ...
    try {
        ...
        const product = await Product.findByIdAndUpdate(productId, { images: imagesPaths }, { new: true } // Third param for returning the updated items
        );
        // If product not found
        if (!product) {
            return res.status(404).json({
        ...
        console.error(`Error updating gallery of product with ID '${productId}': ${error.message || error}`, { uploadedFiles: images.map(img => img.filename) });
        return res.status(500).json({
            success: false,
